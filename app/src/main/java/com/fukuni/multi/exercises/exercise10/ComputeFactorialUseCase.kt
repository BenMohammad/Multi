...
class ComputeFactorialUseCase {
        ...
        return withContext(Dispatchers.IO) {
            try {
                withTimeout(timeMillis = timeout.toLong()) {
                    val computationalRanges = getComputationalRanges(argument)
                    val partialProductForRanges = computePartialProducts(computationalRanges)
    ...
    private fun getComputationalRanges(argument : Int) : Array<ComputationRange> {
        ...
        val threadComputationRanges = Array(numberOfThreads) { ComputationRange(0,0) }
        ...
        for(i in numberOfThreads - 1 downTo 0) {
            threadComputationRanges[i] = ComputationRange(
                    nextComputationalRangeEnd - computationRangeSize + 1,
    ...
    private suspend fun computePartialProducts(computationRange: Array<ComputationRange>) : List<BigInteger> = coroutineScope {
